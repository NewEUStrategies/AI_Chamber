...
/** Objasnienia torów planu — co dana barwa mówi o wpisie. */
const TRACK_INFO: Record<keyof typeof ROADMAP_TRACKS, string> = {
  zasieg:
    'Tor zasięgu: działania, które zwiększają liczbę osób, które w ogóle stykają się z izbą — treści, wyszukiwarki, AI.',
  popyt:
    'Tor popytu: działania, które zamieniają zasięg w zapytania o członkostwo — formularz, outbound, aktywacja sieci.',
  wiarygodnosc:
    'Tor wiarygodności: działania, które sprawiają, że marka wygląda na wiarygodną — profil linkowy, publikacje, cytowania.',
  produkt:
    'Tor produktu: praca nad samą ofertą członkowską — pakiety, ceny, struktura programu.',
};
      ...
      <div className="mb-4 flex flex-wrap gap-x-4 gap-y-1.5 text-[11px] font-semibold text-slate-500">
        {(Object.keys(ROADMAP_TRACKS) as (keyof typeof ROADMAP_TRACKS)[]).map((t) => {
          const legendShow = (e: { clientX: number; clientY: number }) =>
            tip.show(e, [], ROADMAP_TRACKS[t], TRACK_INFO[t]);
          return (
            <span
              key={t}
