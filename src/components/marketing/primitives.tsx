          ...
          {data.map((d) => {
            ...
            return (
              <div
                ...
                className="flex cursor-default flex-col items-stretch gap-1 rounded-[6px] py-1.5 outline-none transition-colors hover:bg-slate-50 focus-visible:bg-slate-50 sm:flex-row sm:items-center sm:gap-3"
              ...
              >
                {/*
                 * Etykieta nigdy nie jest ucinana. Na szerokim ekranie zostaje
                 * stałą kolumną wyrównaną do prawej, bo wyrównana krawędź
                 * pozwala czytać nazwy jedna pod drugą; na wąskim przenosi się
                 * nad pasek i zawija, bo stała kolumna 168–200 px zabierała
                 * wtedy połowę ekranu i kończyła się wielokropkiem.
                 */}
                <span
                  className="break-words text-left text-[12px] font-bold leading-snug text-chamber-navy sm:w-[var(--mkt-label-w)] sm:shrink-0 sm:text-right"
                  style={{ '--mkt-label-w': `${labelWidth}px` } as CSSProperties}
                >
                  {d.label}
                </span>
                {/*
                 * Pasek i liczba trzymają się razem. Na wąskim ekranie tworzą
                 * własny wiersz pod etykietą; od 640 px opakowanie znika
                 * (`contents`) i oba wracają jako bezpośrednie komórki wiersza,
                 * więc układ szeroki zostaje dokładnie taki jak był.
                 */}
